...
from datasets import random_ID_v2
import model
import pdb
import os
...
def define():
  ...
  flags.DEFINE_string('dataset_name', 'random_ID_v2',
  ...
  flags.DEFINE_string('checkpoint', 'models/random_ID_v2/model.ckpt-669418',
  ...
  flags.DEFINE_float('learning_rate', 0.002, #0.0004
                     'learning rate')

  flags.DEFINE_string('optimizer', 'momentum',
                      'the optimizer to use')

  flags.DEFINE_float('momentum', 0.75,
...
def create_dataset(split_name):
  return random_ID_v2.get_split(split_name, dataset_dir=FLAGS.dataset_dir)

def get_charset():
  charsets = {}
  with open(os.path.join(random_ID_v2.DEFAULT_DATASET_DIR, random_ID_v2.DEFAULT_CONFIG["charset_filename"]), "r") as f:
    for row in f:
      split_row = row[:-1].split("\t")
      charsets[split_row[0]] = split_row[1]
  return charsets
